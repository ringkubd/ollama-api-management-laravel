...
@section('content')
    <div class="mb-6">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-3xl font-bold">Edit API Key</h1>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <form action="{{ route('admin.api-keys.update', $apiKey->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $apiKey->name) }}" class="border border-gray-300 rounded-lg p-2 w-full @error('name') border-red-500 @enderror" required>
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="3" class="border border-gray-300 rounded-lg p-2 w-full @error('description') border-red-500 @enderror">{{ old('description', $apiKey->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        @foreach($models as $model)
                            <div class="mb-2">
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="allowed_models[]" value="{{ $model->id }}" class="form-checkbox h-5 w-5 text-blue-600"
                                        {{ in_array($model->id, old('allowed_models', $apiKey->allowed_models ?? [])) ? 'checked' : '' }}>
                ...
                <div class="mb-6">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_active" class="form-checkbox h-5 w-5 text-blue-600" {{ old('is_active', $apiKey->is_active) ? 'checked' : '' }}>
